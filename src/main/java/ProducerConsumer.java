import java.util.LinkedList;
import java.util.List;

// This class has a list, producer (adds items to list
// and consumber (removes items).
public class ProducerConsumer implements EKGObserver {
    private EKGSensor EKGSensor = new EKGSensor(0);
    // Create a list shared by producer and consumer
    // Size of list is 2.
    LinkedList<EKGDTO> listGUI = new LinkedList<EKGDTO>();
    LinkedList<EKGDTO> listDB = new LinkedList<EKGDTO>();
    int capacity = 100;
    private EKGListener listener;


    // Function called by producer thread
    public void produce() throws InterruptedException {

        while (true) {
            synchronized (this) {
                // producer thread waits while list
                // is full
                while (listGUI.size() == capacity && listDB.size() == capacity)
                    wait();
                List<EKGDTO> value = EKGSensor.getData();
                // to insert the jobs in the list
                if (value != null) {
                    for (EKGDTO i : value) {
                        listGUI.add(i);
                        listDB.add(i);
                        System.out.println("Producer produced- "
                                + i);
                    }
                }
                // notifies the consumer thread that
                // now it can start consuming
                notify();

                // makes the working of program easier
                // to understand
            }
            //Thread.sleep(10);
        }
    }

    // Function called by consumer thread
    public void consumeGUI() throws InterruptedException {
        while (true) {
            LinkedList<EKGDTO> guiConsumedList;
            synchronized (this) {
                // consumer thread waits while list
                // is empty
                while (listGUI.size() < 50)
                    wait();

                // to retrive the ifrst job in the list
                //listGUI.removeAll(guiConsumedList);
                guiConsumedList = listGUI;
                listGUI = new LinkedList<EKGDTO>();
                if (listener != null) {
                    listener.notify(guiConsumedList);
                }

                // Wake up producer thread
                notify();

                // and sleep
            }
            //System.out.println("Consumer GUIconsumed-");
            for (EKGDTO i : guiConsumedList) {
                System.out.println("GUInummer " + i);
            }
            //Thread.sleep(10);
        }
    }

    public void consumeDB() throws InterruptedException {
        while (true) {
            LinkedList<EKGDTO> dbConsumedList;
            synchronized (this) {
                // consumer thread waits while list
                // is empty
                while (listDB.size() < 50)
                    wait();

                // to retrive the ifrst job in the list
                //listGUI.removeAll(dbConsumedList);
                dbConsumedList = listDB;
                listDB = new LinkedList<EKGDTO>();

                // Wake up producer thread
                notify();

                // and sleep
            }
            //System.out.println("Consumer DBconsumed-");
            for (EKGDTO i : dbConsumedList) {
                System.out.println("DBNummer " + i);
            }
            //…Thread.sleep(10);
        }
    }

    @Override
    public void registerObserver(EKGListener listener) {
        this.listener = listener;
    }
}